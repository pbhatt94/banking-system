...
public class ClosedAccountsDAO extends GenericDAO<ClosedAccounts> {
	...
	public ClosedAccounts getClosedAccount(String username) throws ClassNotFoundException, SQLException {
		String query = String.format("SELECT * FROM %s WHERE %s = '%s'", 
				ClosedAccountsConstants.TABLE_NAME,
				ClosedAccountsConstants.COLUMN_USERNAME,
				username
				);
		return get(query);
	}
	...
	public boolean addClosedAccounts(ClosedAccounts closedAccounts) throws ClassNotFoundException, SQLException {
		String query = String.format(
		        ...
		        "VALUES ('%s', '%s')",
		        ClosedAccountsConstants.TABLE_NAME,
		        ClosedAccountsConstants.COLUMN_ID,
		        ClosedAccountsConstants.COLUMN_USERNAME,
		        closedAccounts.getId(),
		        closedAccounts.getUsername()
		    );
