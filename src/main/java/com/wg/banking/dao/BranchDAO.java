...
public class BranchDAO extends GenericDAO<Branch> {
	...
	public Branch getBranchById(String branchId) throws ClassNotFoundException, SQLException {
		String query = String.format("SELECT * FROM %s WHERE %s = '%s'", BranchConstants.BRANCH_TABLE_NAME, BranchConstants.BRANCH_ID_COLUMN, branchId);
		return get(query);
	}
	
	public Branch getBranch(String managerId) throws ClassNotFoundException, SQLException {
		String query = String.format("SELECT * FROM %s WHERE %s = '%s'", BranchConstants.BRANCH_TABLE_NAME, BranchConstants.BRANCH_MANAGER_COLUMN, managerId);
		return get(query);
	}
	...
	public boolean addBranch(Branch branch) throws ClassNotFoundException, SQLException {
		String query = String.format(
		        ...
		        BranchConstants.BRANCH_TABLE_NAME,
		        BranchConstants.BRANCH_ID_COLUMN,
		        BranchConstants.BRANCH_NAME_COLUMN,
		        BranchConstants.BRANCH_ADDRESS_COLUMN,
		        BranchConstants.BRANCH_MANAGER_COLUMN,
		        BranchConstants.CREATED_AT_COLUMN,
		        BranchConstants.UPDATED_AT_COLUMN,
		        branch.getBranchId(),
		        branch.getBranchName(),
		        branch.getBranchAddress(),
		        branch.getBranchManagerId(),
		        new java.sql.Date(branch.getCreatedAt().getTime()),
		        new java.sql.Date(branch.getUpdatedAt().getTime())
		    );
